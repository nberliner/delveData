...
class UNHCRdata(Settings):
    ...
    def _showCountryByOrigin(self, origin_country):
        ...
        tmpData = self.data[ self.data["Origin"] == origin_country ]
        tmpData = tmpData.groupby(["Year"], sort=True).sum()
    ...
    def _showCountryDirect(self, destination_country, origin_country):
        ...
        x  = tmpData["Year"]
        y1 = tmpData["Refugees (incl. refugee-like situations)"]
        y2 = tmpData["Asylum-seekers (pending cases)"]
        y3 = tmpData["Returned refugees"]
        y4 = tmpData["Internally displaced persons (IDPs)"]
        y5 = tmpData["Returned IDPs"]
        y6 = tmpData["Stateless persons"]
